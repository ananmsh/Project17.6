...
public class PurchaseGui  {
    ...
    void CmbCity(ActionEvent event) throws InterruptedException {
    	...
    	getprice.add("111"); // HAS TO BE CUSTOMER ID
    	try {
    ...
    void PurchaseBtn(ActionEvent event) {
    	if(!(OneTimeRad.isDisabled()))
    	{
            ...
            purchase.setCustomerid("111");; // HAS TO BE CUSTOMER ID
    	...
    	else
    	{
    		...
    		purchase.setCityname((String)ComboBoxCity.getSelectionModel().getSelectedItem());
            purchase.setCustomerid("111");; // HAS TO BE CUSTOMER ID
            purchase.setOneTimeDate(LocalDate.now());
    ...
    void initialize() {
    	...
    	endDatePicker.setValue(in6months); // Max date available will be 2015-01-01
